# Completions for Claude Code (claude). Flags and subcommands mirror
# `claude --help`; the claude-code-update skill keeps them in sync with
# new CLI releases. Session lookup for --resume is left to the ,cr and
# ,cs pickers.
...
complete -c claude -n __claude_no_subcommand -s r -l resume -x -d 'Resume a conversation by session ID'
